...
const toast = m=>{$('#aviso').textContent=m;show('#aviso');setTimeout(()=>show('#aviso',false),3500);}
const mmss2sql = v=>v&&/^\d{1,2}:\d{2}$/.test(v)?'00:'+v.padStart(5,'0'):null;
const sql2mmss = v=>v?v.slice(3):'';
const CONTEXTOS=['Estágio','Pecege','Monitoria','Pesquisa/IC','CWS','Outros'];
const ctxNorm = c=>CONTEXTOS.includes(c)?c:'Outros';
...
function md(src=''){
  ...
  return src.split(/\r?\n/).reduce((h,l)=>{
    const m=l.match(/^\s*[-*]\s+(.*)/);
    if(m){ if(!h.endsWith('</ul>'))h+='<ul>'; return h+`<li>${m[1]}</li>`;}
    return h+(h.endsWith('</li>')?'</ul>':'')+(l.trim()?`<p>${l}</p>`:'');
  },'').replace(/<ul>(?![\s\S]*<\/ul>)/,'<ul>'); // fecha lista se necessário
}
...
function openModal(overlay,box){
  show(overlay);show(box);box.querySelector('button, [href], input, textarea, select')?.focus();
}
function closeModal(overlay,box){show(overlay,false);show(box,false);}
function clickOutsideToClose(overlay,box){overlay.onclick=()=>closeModal(overlay,box);}
function escToClose(overlay,box){
  const fn=e=>{if(e.key==='Escape'){closeModal(overlay,box);document.removeEventListener('keydown',fn);} };
...
$('#form').onsubmit=async e=>{
  ...
  const nova={
    ...
    contexto:ctxNorm(f.contexto.value),responsavel:f.responsavel.value,
...
function openRead(t){
  ...
  $('#readContexto').textContent='Contexto: '+ctxNorm(t.contexto);
  $('#readResp').textContent    ='Resp: '+(t.responsavel||'-');
  openModal('#readOverlay','#readModal');
  clickOutsideToClose('#readOverlay','#readModal');
  escToClose('#readOverlay','#readModal');
}
...
function openEdit(t){
  ...
  f.prioridade.value=t.prioridade||'normal';f.contexto.value=ctxNorm(t.contexto);
  f.responsavel.value=t.responsavel||'';
  openModal('#overlay','#editModal');clickOutsideToClose('#overlay','#editModal');escToClose('#overlay','#editModal');
  $('#btn-excluir-modal').onclick=()=>{tarefaParaExcluir=t;openModal('#modalExcluir','#excluirBox');clickOutsideToClose('#modalExcluir','#excluirBox');escToClose('#modalExcluir','#excluirBox');};
}
...
$('#btn-concluidas').onclick=async()=>{
  openModal('#modal-concluidas','#concluidasBox');clickOutsideToClose('#modal-concluidas','#concluidasBox');escToClose('#modal-concluidas','#concluidasBox');
  ...
  $('#table-concluidas tbody').innerHTML=data.map(r=>`<tr><td>${r.task}</td><td>${ctxNorm(r.contexto)}</td><td>${r.responsavel||''}</td><td>${new Date(r.concluded_at).toLocaleString()}</td></tr>`).join('');
};
...
$('#btnConfirmarExcluir').onclick=async()=>{
  ...
  await supabase.from('excluidos').insert([{todo_id:tarefaParaExcluir.id,task:tarefaParaExcluir.task,contexto:ctxNorm(tarefaParaExcluir.contexto),responsavel:tarefaParaExcluir.responsavel}]);
  await supabase.from('todos').delete().eq('id',tarefaParaExcluir.id);
  closeModal('#modalExcluir','#excluirBox');tarefaParaExcluir=null;toast('Tarefa excluída!');render();
};
