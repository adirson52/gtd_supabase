      ...
      onEnd: async evt => {
        ...
        for (let i = 0; i < cards.length; i++) {
          ...
          if (dest === 'concluido') {
            ...
            await supabase
              .from('concluded')
              .insert([{ todo_id: id, task: t.task, contexto: t.contexto, responsavel: t.responsavel }]);
          }
        }
        toast('Tarefa movida!');
        render();
...
$('#btn-concluidas').onclick = async () => {
  ...
  $('#table-concluidas tbody').innerHTML = data
    .map(r => `
      <tr>
        <td>${r.task}</td>
        <td>${normalizaContexto(r.contexto)}</td>
        <td>${r.responsavel || ''}</td>
        <td>${new Date(r.concluded_at).toLocaleString()}</td>
      </tr>`)
    .join('');
};
$('#closeConcluidas').onclick = () => {
  show('#modal-concluidas', false);
  show('#concluidasBox', false);
};

/* ---------- DELETE ---------- */
$('#btnCancelarExcluir').onclick = () => {
  show('#modalExcluir', false);
  show('#excluirBox', false);
};
$('#btnConfirmarExcluir').onclick = async () => {
  ...
  await supabase.from('excluidos').insert([{
    ...
    contexto: normalizaContexto(tarefaParaExcluir.contexto),
