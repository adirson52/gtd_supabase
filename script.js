...
/* ============ MARCAÇÃO SIMPLES (tachado & listas) ============ */
...
$('#form').onsubmit = async e=>{
  ...
  const { data:max } = await supabase.from('todos')
    .select('ordem').eq('status',f.status.value).order('ordem',{ascending:false}).limit(1).single();
  const nova = {
    task:f.titulo.value,
...
function openRead(t){
  ...
  $('#readDescricao').innerHTML = markdownToHtml(t.descricao||'(sem descrição)');
...
$('#btn-concluidas').onclick = async ()=>{
  show('#modal-concluidas'); $('#table-concluidas tbody').innerHTML='<tr><td colspan="4">Carregando…</td></tr>';
  const since=new Date(Date.now()-7*864e5).toISOString();
  const{data,error}=await supabase
    .from('concluded').select('*').gte('concluded_at',since).order('concluded_at',{ascending:false});
