/* ============= UTILIDADES ============= */
const $  = s => document.querySelector(s);                 // seletor curto – **uma** só vez
const $$ = s => [...document.querySelectorAll(s)];         // lista de seletores
...
async function render () {
  ...
  /* ---------- filtra 3 + recentes em “concluído” ---------- */
...
function openEdit(t){
  ...
  /* botão Excluir (um só) */
  if(!$('#btn-excluir-modal')){
    const btn=document.createElement('button');
    btn.id='btn-excluir-modal'; btn.type='button'; btn.textContent='Excluir';
    btn.className='read-close'; btn.style.cssText='background:#d32f2f;margin-top:1rem;width:100%';
    btn.onclick = ()=>{ tarefaParaExcluir=t; show('#modalExcluir'); };
    f.appendChild(btn);
  } else {
...
$('#btnConfirmarExcluir').onclick = async ()=>{
  ...
  const { error:logErr } = await supabase.from('excluidos').insert([{
  ...
  if(logErr||delErr){ toast('Erro ao excluir'); console.error(logErr||delErr); }
  else{ toast('Tarefa excluída!'); render(); }
};
