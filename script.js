...
const toast=msg=>{const el=document.getElementById('aviso');el.textContent=msg;show('aviso',true);setTimeout(()=>show('aviso',false),3500)};
...
async function render(){
  const {data:tarefas,error}=await supabase.from('todos').select('*').order('status',{ascending:true}).order('ordem',{ascending:true});
  if(error){console.error(error);return;}

  const labels={urgente:'Urgente',nao_iniciado:'Não Iniciado',em_andamento:'Em Andamento',com_data:'Com Data',concluido:'Concluído'};
  const kanban=document.getElementById('kanban');
  kanban.innerHTML=Object.entries(labels).map(([k,l])=>`<div class="column" data-col="${k}"><h2>${l}</h2></div>`).join('');

  const recentIds=tarefas.filter(t=>t.status==='concluido').sort((a,b)=>new Date(b.moved_at)-new Date(a.moved_at)).slice(0,3).map(t=>t.id);
  ...
  kanban.querySelectorAll('.card').forEach(card=>{
    ...
    card.onclick=e=>{if(e.target.classList.contains('move-btn')||e.target.classList.contains('edit-btn'))return;openRead(dados);}
  });
...
document.getElementById('form').addEventListener('submit',async e=>{
  ...
  const nova={task:f.titulo.value,descricao:f.descricao.value||null,status:f.status.value,tempo_estimado:mmss2sql(f.tempo_estimado.value),prioridade:f.prioridade.value,contexto:f.contexto.value,responsavel:f.responsavel.value,ordem:(max?.ordem||0)+1,user_id:'00000000-0000-0000-0000-000000000000'};
...
function openEdit(t){
  ...
  if(!btn){
    ...
    btn.onclick=()=>{tarefaParaExcluir=t;show('modalExcluir');document.querySelector('#modalExcluir .read-modal').style.display='block';};
    f.appendChild(btn);
  }else{
    btn.onclick=()=>{tarefaParaExcluir=t;show('modalExcluir');document.querySelector('#modalExcluir .read-modal').style.display='block';};
  }
...
editForm.addEventListener('submit',async e=>{
  e.preventDefault();
  const f=e.target;
  const upd={task:f.titulo.value,descricao:f.descricao.value||null,status:f.status.value,tempo_estimado:mmss2sql(f.tempo_estimado.value),prioridade:f.prioridade.value,contexto:f.contexto.value,responsavel:f.responsavel.value};
...
document.getElementById('btn-concluidas').onclick=async ()=>{
  ...
  const {data,error}=await supabase.from('concluded').select('*').gte('concluded_at',limite.toISOString()).order('concluded_at',{ascending:false});
  if(error){tbody.innerHTML=`<tr><td colspan="4" style="color:red">Erro: ${error.message}</td></tr>`;return;}
  if(!data.length){tbody.innerHTML=`<tr><td colspan="4">Nenhuma tarefa concluída na última semana.</td></tr>`;return;}
  tbody.innerHTML=data.map(r=>`<tr><td>${r.task}</td><td>${r.contexto||''}</td><td>${r.responsavel||''}</td><td>${new Date(r.concluded_at).toLocaleString()}</td></tr>`).join('');
};

/* ============= EXCLUIR ========= */
btnCancelarExcluir.onclick=()=>{show('modalExcluir',false);document.querySelector('#modalExcluir .read-modal').style.display='none';};
btnConfirmarExcluir.onclick=async ()=>{
  if(!tarefaParaExcluir)return;
  const {error:logErr}=await supabase.from('excluidos').insert([{todo_id:tarefaParaExcluir.id,task:tarefaParaExcluir.task,contexto:tarefaParaExcluir.contexto,responsavel:tarefaParaExcluir.responsavel}]);
  const {error:delErr}=await supabase.from('todos').delete().eq('id',tarefaParaExcluir.id);
  show('modalExcluir',false);document.querySelector('#modalExcluir .read-modal').style.display='none';tarefaParaExcluir=null;
  if(logErr||delErr){toast('Erro ao excluir');console.error(logErr||delErr);}else{toast('Tarefa excluída!');render();}
};
